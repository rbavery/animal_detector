...
station_results_folder_name = st.text_input(
    "Enter a name for the results folder where output detection images and statistics will be stored: "
)
station_results_folder_name = os.path.join("results", station_results_folder_name)
...
if os.path.exists(station_results_folder_name):
    st.warning(
        "Warning: The results folder {} already exists and would be overwritten. Please choose a different name.".format(
            station_results_folder_name
        )
    )
    st.stop()
...
for i in results:
    if "detections" in i:
        high_conf_dets = filter(lambda det: det["conf"] >= 0.9, i["detections"])
        if len(high_conf_dets) < 4:
...
st.text(
    f"Running detections is done! Results are saved at {station_results_folder_name}. Restart the app to run another station."
)
